...
public:
    ...
    void load(const juce::File& onnxFile) {
        ...
        ctx.setSize(1, (int)cfg.context); ctx.clear(); tmp.setSize(1, 0);
    }
    void process(float* samples, int num) {
        if (!session) return; const int T = (int)cfg.context + num;
        tmp.setSize(1, T);
        tmp.copyFrom(0, 0, ctx, 0, (int)cfg.context);
        ...
        const char* inNames[] = { inputName.c_str() }; const char* outNames[] = { outputName.c_str() };
        ...
        std::memcpy(ctx.getWritePointer(0), dataIn + (T - (int)cfg.context), sizeof(float)*(size_t)cfg.context);
    }
private:
    std::unique_ptr<Ort::Env> env; std::unique_ptr<Ort::Session> session; std::string inputName, outputName;
